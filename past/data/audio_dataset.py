...
from audiocraft.data.audio_dataset import BaseInfo, AudioMeta, DEFAULT_EXTS, load_audio_meta, find_audio_files, save_audio_meta, _resolve_audio_meta
...
class AudioMeta(BaseInfo):
    ...
    transcriptions_alignment_path: tp.Optional[str] = None
    phones_path: tp.Optional[str] = None
    ...
    @property
    def phones(self) -> tp.Optional[tp.List[str]]:
        """Returns the phones from the phones_path if it exists."""
        if self.phones_path is None:
            return None
        df = pd.read_csv(self.phones_path, sep=' ', header=None, names=['start_offset', 'end_offset', 'phone'])
        return [(row['phone'], row['start_offset'], row['end_offset']) for _, row in df.iterrows()]

    @property
    def transcriptions_alignment(self) -> tp.Optional[tp.List[tp.Tuple[float, float, str]]]:
        if self.transcriptions_alignment_path is None:
            return None
        df = pd.read_csv(self.transcriptions_alignment_path, sep=',', header=None, names=['char', 'start_offset', 'end_offset'])
        return [(row['char'], row['start_offset'], row['end_offset']) for _, row in df.iterrows()]
...
def load_audio_meta(path: tp.Union[str, Path], resolve: bool = True, fast: bool = True, read_trans: bool = True) -> tp.List[AudioMeta]:
...
class AudioDataset:
    ...
    def sample_file(self, index: int, rng: torch.Generator) -> AudioMeta:
        ...
        if self.permutation_on_files:
            ...
            permutation = AudioDataset._get_file_permutation(len(self.meta), permutation_index, self.shuffle_seed)
    ...
    def __getitem__(self, index: int) -> tp.Union[torch.Tensor, tp.Tuple[torch.Tensor, SegmentInfo]]:
        ...
        if self.segment_duration is None:
            ...
            for retry in range(self.max_read_retry):
                ...
                if self.win_len_seconds is not None and max_seek == 0:
                    init_point = torch.rand(1, generator=rng).item() * self.win_len_seconds
                else:
                    init_point = 0.0
                seek_time = torch.rand(1, generator=rng).item() * max_seek + init_point
                try:
    ...
    def from_meta(cls, root: tp.Union[str, Path], **kwargs):
        ...
        if max_duration is not None:
            meta = [m for m in meta if m.duration <= max_duration]
        return cls(meta, **kwargs)

    @classmethod
    def from_path(cls, root: tp.Union[str, Path], minimal_meta: bool = True, exts: tp.List[str] = DEFAULT_EXTS, **kwargs):
...
def main():
    ...
    parser = argparse.ArgumentParser(prog='audio_dataset', description='Generate .jsonl files by scanning a folder.')
    parser.add_argument('root', help='Root folder with all the audio files')
    parser.add_argument('output_meta_file', help='Output file to store the metadata, ')
    parser.add_argument(
        '--complete', action='store_false', dest='minimal', default=True, help='Retrieve all metadata, even the one that are expansive ' 'to compute (e.g. normalization).'
    )
    parser.add_argument('--resolve', action='store_true', default=False, help='Resolve the paths to be absolute and with no symlinks.')
    parser.add_argument('--workers', default=10, type=int, help='Number of workers.')
    args = parser.parse_args()
    meta = find_audio_files(args.root, DEFAULT_EXTS, progress=True, resolve=args.resolve, minimal=args.minimal, workers=args.workers)
