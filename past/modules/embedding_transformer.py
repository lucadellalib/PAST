...
class EmbeddingTransformer(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            self.drop_transformer_prob + self.drop_skip_connection_prob <= 1.0
        ), "Sum of drop probabilities should be less than 1"
    ...
    def _apply_transformer(self, embeddings, att_mask=None):
        ...
        if self.causal:
            causal_mask = torch.triu(
                torch.ones(
                    embeddings.size(1), embeddings.size(1), device=embeddings.device
                ),
                diagonal=1,
            ).bool()
            if att_mask is not None:
                encoder_attention_mask = (
                    causal_mask.unsqueeze(0) & (~att_mask.unsqueeze(1))
                ).float()
            else:
                encoder_attention_mask = causal_mask.float()
        else:
            encoder_attention_mask = (
                (~att_mask).float() if att_mask is not None else None
            )
        emb_bert = self.transformer_encoder(
            inputs_embeds=embeddings, encoder_attention_mask=encoder_attention_mask
        )
    ...
    def run_transformer_in_chunks(self, embeddings):
        ...
        for start in range(0, T, step):
            ...
            if last_len < M:
                ...
                embeddings_windows[-1] = torch.cat(
                    [embeddings_windows[-1], emb_pad_vector], dim=1
                )
                mask_pad_vector = torch.ones(B, M - last_len, device=device).bool()
                mask_windows[-1] = torch.cat([mask_windows[-1], mask_pad_vector], dim=1)
        embeddings_len = torch.stack(
            embeddings_windows, dim=1
        )  # Shape: (batch_size, num_windows, M, feature_dim)
        embeddings_len = embeddings_len.view(
            -1, M, C
        )  # Shape: (batch_size * num_windows, M, feature_dim)

        att_mask_len = torch.stack(
            mask_windows, dim=1
        )  # Shape: (batch_size, num_windows, M)
        ...
        for i, start in enumerate(range(0, T, step)):
            merged_signal[:, start : start + M] += emb_after[
                :, i, : (M if start + M <= T else T - start)
            ]
    ...
    def calc_embeddings_after_drop_and_mask(self, embeddings, trans_output):
        ...
        new_e = (
            new_e
            + torch.where(p < self.drop_skip_connection_prob, trans_output, embeddings)
            * 0.5
        )
        embeddings = (
            new_e
            + torch.where(
                (1 - p) < self.drop_transformer_prob, embeddings, trans_output
            )
            * 0.5
        )
        return embeddings
    ...
    def forward(self, embeddings):
        ...
        if not self.do_skip_connection:
            ...
            embeddings = self.calc_embeddings_after_drop_and_mask(
                embeddings, trans_output
            )
