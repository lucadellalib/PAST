...
IND2LAB = {v: k for k, v in LAB2IND.items()}
EXTRAS = {
    'starting_index': '0',
    'unk_label': '1',
    'blank_label': '<blank>',
...
class CategoricalEncoder:
                ...
                if not self.load_if_possible(path):
                    ...
                    for didataset in from_didatasets:
                        if output_key is None:
                            raise ValueError("Provide an output_key for " "DynamicItemDataset")
    ...
    def enforce_label(self, label, index):
        ...
        if moving_other:
            logger.info(f"Moving label {repr(saved_label)} from index " f"{index}, because {repr(label)} was put at its place.")
    ...
    def encode_label(self, label, allow_unk=True):
        ...
        try:
            ...
            if hasattr(self, "unk_label") and allow_unk:
                ...
                raise KeyError(f"Unknown label {label}, and explicitly " "disallowed the use of the existing unk-label")
            elif not hasattr(self, "unk_label") and allow_unk:
                raise KeyError(f"Cannot encode unknown label {label}. " "You have not called add_unk() to add a special " "unk-label for unknown labels.")
            else:
                raise KeyError(f"Couldn't and wouldn't encode unknown label " f"{label}.")
    ...
    def decode_ndim(self, x):
        ...
        self._assert_len()
        # Recursively operates on the different dimensions.
        return [self.ind2lab[n] for n in x]

    def decode_ndim_pairs(self, x):
        """Decodes an arbitrarily nested iterable to a list of labels.
    ...
    def load(self, path):
        ...
        if self.lab2ind:
            clsname = self.__class__.__name__
            logger.info(f"Load called, but {clsname} is not empty. " "Loaded data will overwrite everything. " "This is normal if there is e.g. an unk label defined at init.")
        if path is None:
            lab2ind, ind2lab, extras = LAB2IND, IND2LAB, EXTRAS
        else:
    ...
    def load_if_possible(self, path, end_of_epoch=False):
        ...
        try:
            ...
            logger.debug(f"Would load categorical encoding from {path}, " "but file doesn't exist yet.")
            return False
        except (ValueError, SyntaxError):
            logger.debug(f"Would load categorical encoding from {path}, " "and file existed but seems to be corrupted or otherwise couldn't load.")
            return False
    ...
    def _assert_len(self):
        ...
        if hasattr(self, "expected_len"):
            ...
            if real_len != self.expected_len:
                raise RuntimeError(f".expect_len({self.expected_len}) was called, " f"but {real_len} categories found")
        else:
            logger.debug(
...
class TextEncoder(CategoricalEncoder):
    ...
    def limited_labelset_from_iterable(self, iterable, sequence_input=True, n_most_common=None, min_count=1):
        """Change default for sequence_input to True."""
        return super().limited_labelset_from_iterable(iterable, sequence_input=True, n_most_common=None, min_count=1)
    ...
    def add_bos_eos(
        ...
        if bos_label == eos_label:
            logger.debug("BOS and EOS labels are the same so using just one sentence " "boundary label")
...
class CTCTextEncoder(TextEncoder):
    ...
    def collapse_labels(self, x, merge_repeats=True):
        ...
        if merge_repeats:
            return [label for i, label in enumerate(x) if (i == 0 or label != x[i - 1]) and label != self.blank_label]
        else:
    ...
    def collapse_indices_ndim(self, x, merge_repeats=True):
        ...
        if merge_repeats:
            return [index for i, index in enumerate(x) if (i == 0 or index != x[i - 1]) and index != blank_index]
        else:
