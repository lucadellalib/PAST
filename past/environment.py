...
class AudioCraftEnvironment:
    ...
    def __init__(self) -> None:
        ...
        config_path = os.getenv(
            "AUDIOCRAFT_CONFIG",
            Path(__file__).parent.parent.joinpath("config/teams", self.team).with_suffix(".yaml"),
        )
    ...
    def get_slurm_partitions(cls, partition_types: tp.Optional[tp.List[str]] = None) -> str:
        ...
        cluster_config = cls.instance()._get_cluster_config()
        partitions = [cluster_config["partitions"][partition_type] for partition_type in partition_types]
    ...
    def resolve_reference_path(cls, path: tp.Union[str, Path]) -> Path:
        ...
        if path.startswith("//reference"):
            ...
            assert reference_dir.exists() and reference_dir.is_dir(), f"Reference directory does not exist: {reference_dir}."
